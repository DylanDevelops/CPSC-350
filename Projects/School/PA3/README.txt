AUTHOR INFO
Full name: Dylan M. Ravel
Student ID: 2445987
Chapman Email: ravel@chapman.edu
Course number and section: CPSC-350-03
Assignment or exercise number: PA3: Do You See What I See?

ERRORS 
There are no errors from what I have observed so far.

SOURCES
Below are all the sources I used throughout my project. Next to each link is how it helped me specifically with the project.
    - https://dreel.notion.site/Templated-Stack-2ef716fd3f834ae999d5dc10b103c590 (This was the starting point for my MonoStack)
    - https://www.w3schools.com/cpp/cpp_break.asp (Understanding how the "continue" keyword can be used in while loops)
    - Referenced class material and previous projects for help with file manipulation in C++
    - 

RUNNING INSTRUCTIONS
Compile the file as so: "g++ *.cpp -o program.out"
Run the file as so: "./program.out INPUT_FILE.txt" ( !!! Make sure to include file extensions !!! )